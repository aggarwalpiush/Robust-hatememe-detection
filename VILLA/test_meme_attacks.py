...
def main(opts, checkpoint_dir=None, tuning=False):
    ...
    with logger.catch(reraise=True):
        ...
        val_img_db = all_img_dbs[opts.test_img_db]
        val_txt_db = TxtTokLmdb(opts.test_txt_db, -1)
        ...
        y_pred = [int(r['proba'] > 0.5) for r in results]
        val_log['valid/auroc'] = auroc
        f1 = f1_score(y_true, y_pred, average='macro')
        with open(f'{opts.output_dir}/metric.txt', 'w') as f:
            f.write("auroc: "+str(auroc)+" \n")
            f.write("f1(macro): "+str(f1)+"\n")

        with open(opts.result_file, 'a') as f:
            f.write(str(opts.dataset)+"\t" + str(opts.attack) + "\t"+ str(opts.ocr) + "\t"+str(opts.model)+ "\t"+ str(auroc) + "\t"+ str(f1)+"\n")
        
        with open(f'{opts.output_dir}/pred.json', 'w') as f:
            json.dump(results, f)
        pd.DataFrame.from_dict(results).to_csv(
            f'{opts.output_dir}/pred.csv', index=False)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--result_file', type=str, default="/txt",
                        help="")
    parser.add_argument('--dataset', type=str, default="fb",
                        help="")
    parser.add_argument('--attack', type=str, default="sp",
                        help="")
    parser.add_argument('--model', type=str, default="villa",
                        help="")
