...
class HMTorchDataset(Dataset):
    def __init__(self, dataset_name, split_type, img_adv_attack,text_adv_attack):
        ...
        self.aug_tsv_path = os.path.join(self.global_path,dataset_name,'img_feats',split_type+'_train_contlearn.tsv')
        self.img_feat_aug = load_obj_tsv(self.aug_tsv_path, [])
        ...
        self.imgAugid2data = {}
        for item in self.img_feat_aug:
            self.imgAugid2data[item['img_id']] = item
...
#         if i<10:
#             print(text)
        augmented_text = text_transform(text)[0]
        img_id = self.data[i]['img']
        img_info = self.imgid2data[self.data[i]['img']]
        img_aug_info = self.imgAugid2data[self.data[i]['img']]
        obj_num = img_info['num_boxes']
        feats = img_info['features'].copy()
        feats_aug = img_aug_info['features'].copy()
        boxes = img_info['boxes'].copy()
        boxes_aug = img_aug_info['boxes'].copy()
        feats = feats[:10]
        boxes = boxes[:10]
        feats_aug = feats_aug[:10]
        boxes_aug = boxes_aug[:10]
...
#         assert obj_num == len(boxes) == len(feats)
        ...
        # Normalize the boxes (to 0 ~ 1)
        img_h_aug, img_w_aug = img_aug_info['img_h'], img_aug_info['img_w']
        ...
        if args.num_pos == 5: 
            ...
            image_location_aug = np.zeros((boxes_aug.shape[0], 5), dtype=np.float32)
            image_location_aug[:,:4] = boxes_aug
            image_location_aug[:,4] = (image_location_aug[:,3] - image_location_aug[:,1]) * (image_location_aug[:,2] - image_location_aug[:,0]) / (float(img_w_aug) * float(img_h_aug))
            boxes_aug = image_location_aug
        ...
        # Augment_image box
        boxes_aug = boxes_aug.copy()
        boxes_aug[:, (0, 2)] /= img_w_aug
        boxes_aug[:, (1, 3)] /= img_h_aug
        np.testing.assert_array_less(boxes_aug, 1+1e-5)
        np.testing.assert_array_less(-boxes_aug, 0+1e-5)
        ...
        if args.num_pos == 6:
            ...
            width_aug = (boxes_aug[:, 2] - boxes_aug[:, 0]).reshape(-1,1)
            height_aug = (boxes_aug[:, 3] - boxes_aug[:, 1]).reshape(-1,1)

            boxes_aug = np.concatenate((boxes_aug, width_aug, height_aug), axis=-1)
            ...
            if args.model == "U":
                ...
                boxes_aug = np.concatenate([boxes_aug, boxes_aug[:, 4:5] * boxes_aug[:, 5:]], axis=-1)

        target = torch.tensor(self.data[i]['label'],dtype=torch.float)
        return img_id, feats, boxes, feats_aug, boxes_aug, text, augmented_text, target

class HMEvaluator:
    def __init__(self, dataset):
        self.dataset = dataset

    def dump_tsv(self, id2ans: dict, id2prob: dict, attack, dataset_name):
        #d = {"id": [str(tensor) for tensor in id2ans.keys()], "proba": list(id2prob.values()),
         #   "label": list(id2ans.values())}
        ans = list(id2ans.values())
        probs = list(id2prob.values())
        ...
        with codecs.open(str(dataset_name)+'_'+str(attack)+'_contrastive_0.5_errorAnalysis.tsv', 'w', 'utf-8') as ea_obj:
            for i, val in enumerate(ids):
                ea_obj.write("%s\t%s\t%s\n" %(str(val), str(label[i]), str(probs[i])))
        return 0
