...
class HM:
    def __init__(self, dataset_name, shot, random_choice):
        self.dataset_name = dataset_name
        self.shot = shot
        self.random_choice = random_choice
        self.train_tuple = get_tuple(self.dataset_name,'train_'+str(self.shot)+'_'+str(self.random_choice), bs=args.batch_size, shuffle=True, drop_last=False)
...
#                     print(logit[0, :].detach())
            ...
            if self.valid_tuple is not None:  # Do Validation
                ...
                if roc_auc > best_roc:
                    best_roc = roc_auc
                    best_acc = acc
                    self.save(self.dataset_name+'_'+str(self.shot)+'_'+str(self.random_choice))
    ...
    def save(self, name):
        if args.swa:
            ...
            torch.save(self.model.state_dict(),
                    os.path.join("saved_m/", "%s.pth" % name))
...
def main():
    # Build Class
    hm = HM(args.dataset, args.shot, args.randomNum)
    print ("init complete")
    # Load Model
...
#         print("DO NOT USE VALIDATION")
    ...
    hm.load(os.path.join('saved_m',hm.dataset_name+'_' +str(hm.shot)+'_'+str(hm.random_choice)+'.pth'))
    acc, f1 = hm.evaluate(hm.test_tuple)
    print('==================\n')
    print(hm.dataset_name)
    print(hm.shot)
    print(hm.random_choice)
    print(acc,f1)
    print('==================\n')
#     results, _ = hm.predict(hm.test_tuple)
#     write_test_results(results,args.model)
