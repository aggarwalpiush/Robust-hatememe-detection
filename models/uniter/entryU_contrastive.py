...
from modeling.modeling_bertX import BertLayerNorm, GeLU, BertLayer
from modeling.modeling_albertX import GeLU_new

from modeling.modeling_bertU import BertU
from modeling.modeling_robertaU import RobertaU
...
#         print("ERRORS: ", loading_info["error_msgs"])
    ...
    def forward(self, sents, visual_feats, sents_aug, visual_feats_aug):
        ...
        if self.tr_name.startswith("roberta"):
            ...
            input_ids_aug, img_feats_aug, img_pos_feats_aug, attn_masks_aug, gather_index_aug = self.preprocess_roberta(sents_aug, visual_feats_aug,
                                                                                                    self.num_features,
                                                                                                    self.tokenizer)
            ...
            input_ids_aug, img_feats_aug, img_pos_feats_aug, attn_masks_aug, gather_index_aug = self.preprocess_bert(sents_aug, visual_feats_aug, self.num_features, self.tokenizer)
        ...
        seq_out_aug, _ = self.model(input_ids_aug.cuda(), None, img_feats_aug.cuda(), img_pos_feats_aug.cuda(),
                                            attn_masks_aug.cuda(), gather_index=gather_index_aug.cuda())



        output = self.classifier(pooled_output)

        return output, seq_out, seq_out_aug
