...
from dataloader_contrastive import meme_dataset
import json
import warnings
...
class CNN_roberta_Classifier(nn.Module):
    ...
    def forward(self, image, text, a_image, a_text, label, train):
        ...
        return label_output, merged, a_merged, image, a_image, text, a_text
...
def validation(dl,model):
    ...
    for i, data in enumerate(dl):
        ...
        with torch.no_grad():
            predictions, merged, _, _ , _, _, _  = model(data['image'],
                                            data['text'], 
                                            data['image'],
                                            data['text'],
                                            data['slabel'],False)
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        with tqdm(train_dataloader, unit="batch") as tepoch:
            ...
            for data in tepoch:
                ...
                data['slabel'] = data['slabel'].cuda()
                output, merged, a_merged, image, a_image, text, a_text = model(data['image'],
                                                 data['text'], 
                ...
                cont_loss_img =  contrastive_loss(image,a_image)
                cont_loss_text = contrastive_loss(text,a_text)
                cont_loss = cont_loss_img + cont_loss_text
        ...
        if f1_score_macro > max_acc:
            ...
            torch.save(model.state_dict(),'saved/'+dataset_name+'_contrastive_img_text_0.07'+'.pth')
...
#         model.load_state_dict(torch.load('saved/'+dataset_name+'.pth'))
        ...
        model.load_state_dict(torch.load('saved/'+dataset_name+'_contrastive_img_text_0.07'+'.pth'))
        model.eval()
