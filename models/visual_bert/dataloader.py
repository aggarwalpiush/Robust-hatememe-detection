...
class HMTorchDataset(Dataset):
    def __init__(self, dataset_name, split_type, img_adv_attack,text_adv_attack):
        ...
        self.file_name = os.path.join(self.global_path,dataset_name,'files',split_type+'.json')
    ...
    def __getitem__(self, i: int):
        text_string = 'text'
        #print(self.text_adv_attack)
        if self.text_adv_attack is not None:
            if self.dataset == 'harmeme' and self.text_adv_attack == 's&p_0.4':
                text_string += ('_'+'s&p0.4')
            else:
